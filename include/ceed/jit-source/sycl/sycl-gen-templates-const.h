...
inline void readDofsOffset1d(const CeedInt num_comp, const CeedInt strides_comp, const CeedInt P_1D, const CeedInt num_elem,
  ...
  if (item_id_x < P_1D_const && elem < num_elem) {
    const CeedInt node = item_id_x;
    const CeedInt ind  = indices[node + elem * P_1D_const];
...
inline void readDofsOffset2d(const CeedInt num_comp, const CeedInt strides_comp, const CeedInt P_1D, const CeedInt num_elem,
  ...
  if (item_id_x < P_1D_const && item_id_y < P_1D_const && elem < num_elem) {
    const CeedInt node = item_id_x + item_id_y * P_1D_const;
    const CeedInt ind  = indices[node + elem * P_1D_const * P_1D_const];
...
inline void readDofsOffset3d(const CeedInt num_comp, const CeedInt strides_comp, const CeedInt P_1D, const CeedInt num_elem,
  ...
  if (item_id_x < P_1D_const && item_id_y < P_1D_const && elem < num_elem) {
    for (CeedInt z = 0; z < P_1D_const; ++z) {
      const CeedInt node = item_id_x + P_1D_const * (item_id_y + P_1D_const * z);
      const CeedInt ind  = indices[node + elem * P_1D_const * P_1D_const * P_1D_const];
      for (CeedInt comp = 0; comp < num_comp; ++comp) r_u[z + comp * P_1D_const] = d_u[ind + strides_comp * comp];
    }
  }
}
...
inline void readDofsStrided3d(const CeedInt num_comp, const CeedInt P_1D, const CeedInt strides_node, const CeedInt strides_comp,
  ...
  if (item_id_x < P_1D_const && item_id_y < P_1D_const && elem < num_elem) {
    for (CeedInt z = 0; z < P_1D_const; ++z) {
      const CeedInt node = item_id_x + P_1D_const * (item_id_y + P_1D_const * z);
      const CeedInt ind  = node * strides_node + elem * strides_elem;
      for (CeedInt comp = 0; comp < num_comp; ++comp) r_u[z + comp * P_1D_const] = d_u[ind + comp * strides_comp];
    }
  }
}
...
inline void readSliceQuadsOffset3d(const CeedInt num_comp, const CeedInt strides_comp, const CeedInt Q_1D, const CeedInt num_elem, const CeedInt q,
  ...
  if (item_id_x < Q_1D_const && item_id_y < Q_1D_const && elem < num_elem) {
    const CeedInt node = item_id_x + Q_1D_const * (item_id_y + Q_1D_const * q);
    const CeedInt ind  = indices[node + elem * Q_1D_const * Q_1D_const * Q_1D_const];
    for (CeedInt comp = 0; comp < num_comp; ++comp) r_u[comp] = d_u[ind + strides_comp * comp];
  }
}
...
inline void writeDofsOffset3d(const CeedInt num_comp, const CeedInt strides_comp, const CeedInt P_1D, const CeedInt num_elem,
  ...
  if (item_id_x < P_1D_const && item_id_y < P_1D_const && elem < num_elem) {
    for (CeedInt z = 0; z < P_1D_const; ++z) {
      const CeedInt node = item_id_x + item_id_y * P_1D_const + z * P_1D_const * P_1D_const;
      const CeedInt ind  = indices[node + elem * P_1D_const * P_1D_const * P_1D_const];
      for (CeedInt comp = 0; comp < num_comp; ++comp)
        atomic_fetch_add_explicit(&d_v[ind + strides_comp * comp], r_v[z + comp * P_1D_const], memory_order_relaxed, memory_scope_device);
    }
  }
}
...
inline void writeDofsStrided3d(const CeedInt num_comp, const CeedInt P_1D, const CeedInt strides_node, const CeedInt strides_comp,
  ...
  if (item_id_x < P_1D_const && item_id_y < P_1D_const && elem < num_elem) {
    for (CeedInt z = 0; z < P_1D_const; ++z) {
      const CeedInt node = item_id_x + P_1D_const * (item_id_y + P_1D_const * z);
      const CeedInt ind  = node * strides_node + elem * strides_elem;
      for (CeedInt comp = 0; comp < num_comp; ++comp) d_v[ind + comp * strides_comp] += r_v[z + comp * P_1D_const];
    }
  }
}
...
inline void gradCollo3d(const CeedInt num_comp, const CeedInt Q_1D, const CeedInt q, const private CeedScalar *restrict r_U,
  ...
  for (CeedInt comp = 0; comp < num_comp; ++comp) {
    if (item_id_x < Q_1D_const && item_id_y < Q_1D_const) {
    ...
    if (item_id_x < Q_1D_const && item_id_y < Q_1D_const) {
      ...
      for (CeedInt i = 0; i < Q_1D_const; ++i)
        r_V[comp + 0 * num_comp] += s_G[i + item_id_x * Q_1D_const] * scratch[i + item_id_y * T_1D];  // Contract x direction (X derivative)

      // Y derivative
      r_V[comp + 1 * num_comp] = 0.0;
      for (CeedInt i = 0; i < Q_1D_const; ++i)
        r_V[comp + 1 * num_comp] += s_G[i + item_id_y * Q_1D_const] * scratch[item_id_x + i * T_1D];  // Contract y direction (Y derivative)

      // Z derivative
      r_V[comp + 2 * num_comp] = 0.0;
      for (CeedInt i = 0; i < Q_1D_const; ++i) r_V[comp + 2 * num_comp] += s_G[i + q * Q_1D_const] * r_U[i + comp * Q_1D_const];  // Contract z direction (Z derivative)
    }
...
inline void gradColloTranspose3d(const CeedInt num_comp, const CeedInt Q_1D, const CeedInt q, const private CeedScalar *restrict r_U,
  ...
  for (CeedInt comp = 0; comp < num_comp; ++comp) {
    ...
    if (item_id_x < Q_1D_const && item_id_y < Q_1D_const) {
      for (CeedInt i = 0; i < Q_1D_const; ++i)
        r_V[q + comp * Q_1D_const] += s_G[item_id_x + i * Q_1D_const] * scratch[i + item_id_y * T_1D];  // Contract x direction (X derivative)
    }
    work_group_barrier(CLK_LOCAL_MEM_FENCE);
    ...
    if (item_id_x < Q_1D_const && item_id_y < Q_1D_const) {
      for (CeedInt i = 0; i < Q_1D_const; ++i)
        r_V[q + comp * Q_1D_const] += s_G[item_id_y + i * Q_1D_const] * scratch[item_id_x + i * T_1D];  // Contract y direction (Y derivative)
    }
    work_group_barrier(CLK_LOCAL_MEM_FENCE);
    ...
    if (item_id_x < Q_1D_const && item_id_y < Q_1D_const) {
      for (CeedInt i = 0; i < Q_1D_const; ++i)
        r_V[i + comp * Q_1D_const] += s_G[i + q * Q_1D_const] * r_U[comp + 2 * num_comp];  // PARTIAL contract z direction (Z derivative)
    }
  }
}
