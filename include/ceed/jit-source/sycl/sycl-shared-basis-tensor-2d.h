...
kernel void Interp(const CeedInt num_elem, 
  ...
  for (CeedInt comp = 0; comp < BASIS_NUM_COMP; comp++) {
    ReadElementStrided2d(comp, BASIS_P_1D, num_elem, 1, BASIS_NUM_NODES *num_elem, BASIS_NUM_NODES, d_U, r_U);
    InterpTensor2d(BASIS_P_1D, BASIS_Q_1D, r_U, s_B, r_V, elem_scratch);
    WriteElementStrided2d(comp, BASIS_Q_1D, num_elem, 1, BASIS_NUM_QPTS *num_elem, BASIS_NUM_QPTS, r_V, d_V);
  }
}
...
kernel void InterpTranspose(const CeedInt num_elem, 
  ...
  for (CeedInt comp = 0; comp < BASIS_NUM_COMP; comp++) {
    ReadElementStrided2d(comp, BASIS_Q_1D, num_elem, 1, BASIS_NUM_QPTS* num_elem, BASIS_NUM_QPTS, d_U, r_U);
    InterpTransposeTensor2d(BASIS_P_1D, BASIS_Q_1D, r_U, s_B, r_V, elem_scratch);
    WriteElementStrided2d(comp, BASIS_P_1D, num_elem, 1, BASIS_NUM_NODES * num_elem, BASIS_NUM_NODES, r_V, d_V);
  }
}
...
kernel void Grad(const CeedInt num_elem, 
  ...
  for (CeedInt comp = 0; comp < BASIS_NUM_COMP; comp++) {
    ReadElementStrided2d(comp, BASIS_P_1D, num_elem, 1, BASIS_NUM_NODES * num_elem, BASIS_NUM_NODES, d_U, r_U);
    GradTensor2d(BASIS_P_1D, BASIS_Q_1D, r_U, s_B, s_G, r_V, elem_scratch);
    for (CeedInt dim = 0; dim < 2; ++dim) {
      WriteElementStrided2d(comp + BASIS_NUM_COMP * dim, BASIS_Q_1D, num_elem, 1, BASIS_NUM_QPTS * num_elem, BASIS_NUM_QPTS, r_V + dim, d_V);
    }
  }
}
...
kernel void GradTranspose(const CeedInt num_elem, 
  ...
  for (CeedInt comp = 0; comp < BASIS_NUM_COMP; comp++) {
    for (CeedInt dim = 0; dim < 2; ++dim) {
      ReadElementStrided2d(comp + BASIS_NUM_COMP * dim, BASIS_Q_1D, num_elem, 1, BASIS_NUM_QPTS * num_elem, BASIS_NUM_QPTS, d_U, r_U + dim);
    }
    GradTransposeTensor2d(BASIS_P_1D, BASIS_Q_1D, r_U, s_B, s_G, r_V, elem_scratch);
    WriteElementStrided2d(comp, BASIS_P_1D, num_elem, 1, BASIS_NUM_NODES * num_elem, BASIS_NUM_NODES, r_V, d_V);
  }
}
...
kernel void Weight(const CeedInt num_elem, global const CeedScalar * restrict q_weight_1d, global CeedScalar * restrict d_W) {
  ...
  WriteElementStrided2d(0, BASIS_Q_1D, num_elem, 1, BASIS_NUM_QPTS * num_elem, BASIS_NUM_QPTS, r_W, d_W);
}
